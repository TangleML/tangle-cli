...
@pytest.mark.parametrize("ca_env_var", ["REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE"])
def test_absolute_signed_url_download_verifies_with_environment_ca_trust(
    ca_env_var: str,
    monkeypatch,
    tmp_path,
) -> None:
    """Signed-URL TLS trust comes from the environment, not the Tangle client.

    Asserting the ``verify`` value resolved inside ``requests`` (rather than the
    kwargs handed to ``requests.get``) pins the whole contract: the env CA
    bundle is honored, so self-hosted object stores stay configurable, and any
    future Tangle-specific CA bundle or verification-disable switch that leaked
    into this path would replace the resolved value and fail here.
    """

    ca_bundle = tmp_path / "self-hosted-ca.pem"
    ca_bundle.write_text("", encoding="utf-8")
    out_dir = tmp_path / "out"
    monkeypatch.delenv("REQUESTS_CA_BUNDLE", raising=False)
    monkeypatch.delenv("CURL_CA_BUNDLE", raising=False)
    monkeypatch.setenv(ca_env_var, str(ca_bundle))

    sent: list[dict[str, Any]] = []

    def fake_send(self: Any, request: Any, **kwargs: Any) -> requests.Response:
        sent.append({"headers": dict(request.headers), "verify": kwargs.get("verify")})
        response = requests.Response()
        response.status_code = 200
        response.raw = io.BytesIO(b"signed bytes")
        response.request = request
        response.url = request.url
        return response

    # Patch the transport adapter, not ``requests.get``, so the real session
    # applies its standard environment trust resolution before the send.
    ...
    artifacts = _manager(client).download_result_artifacts("root-exec", out_dir=out_dir)

    assert artifacts["root::model"].read_bytes() == b"signed bytes"
    assert len(sent) == 1
    assert sent[0]["verify"] == str(ca_bundle)
    # No Tangle credential accompanies the off-origin fetch.
    assert not {"Authorization", "Cookie", "X-Api-Key"} & set(sent[0]["headers"])
