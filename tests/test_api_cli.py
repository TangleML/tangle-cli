...
def test_dynamic_operation_connect_error_with_out_of_range_port_is_clean(monkeypatch):
    # httpx accepts an out-of-range port in a request URL, so the error formatter
    # must not raise on it; the unparsable authority is dropped from the message.
    def fake_request(method, url, **kwargs):
        request = httpx.Request(method, url)
        raise httpx.ConnectError("connection refused", request=request)
    ...
    with pytest.raises(SystemExit) as exc_info:
        app(["pipeline-runs", "list", "--base-url", "http://alice:pw@api.test:99999"])

    message = str(exc_info.value)
    assert exc_info.value.code not in (0, None)
    assert "\n" not in message
    assert message.startswith("Failed to reach GET http://<redacted>/api/pipeline_runs/")
    assert "connection refused" in message
    assert "pw" not in message
    assert "99999" not in message


def test_dynamic_operation_http_error_with_unparsable_reflected_url_fails_closed(monkeypatch):
    body = "see https://cb.test:bad/cb?access_token=sk-0123456789 for details"

    def fake_request(method, url, **kwargs):
        return text_response(method, url, body, status_code=502)
...
def test_refresh_connect_error_with_out_of_range_port_is_clean(monkeypatch):
    def fake_get(url, **kwargs):
        raise httpx.ConnectError("connection refused", request=httpx.Request("GET", url))
    ...
    with pytest.raises(SystemExit) as exc_info:
        app(["refresh", "--base-url", "http://api.test:99999"])

    message = str(exc_info.value)
    assert "\n" not in message
    assert message.startswith("Failed to fetch http://<redacted>/openapi.json:")
    assert "connection failed: connection refused" in message
    assert "99999" not in message
