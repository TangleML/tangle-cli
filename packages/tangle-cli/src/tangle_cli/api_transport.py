...
# ``Token`` and ``OAuth`` are also ordinary English words, so whether a matched
# run of scheme words is trusted to carry a credential is judged per match
# against ``_AMBIGUOUS_BARE_SCHEMES`` rather than encoded in the pattern.
_AUTH_SCHEME = (
...
# truncated JSON is scrubbed too. A ``scheme://`` URL start needs no grammar
# exemption here: its would-be field name (``https``, ``x``) is judged
# non-sensitive and declined, while an explicitly sensitive name always loses
# its value -- a credential is not exempted by beginning with slashes
# (``password=//hunter2``). Everything after the separator character is only
# scanned once a separator matched, so scanning is linear in the text length.
# The separator absorbs any whitespace, newlines included: display collapses
# whitespace only after redaction, so a credential on the line below its field
# name must be caught here, before collapsing makes the two adjacent.
_ASSIGNMENT_SEPARATOR_RE = re.compile(r"[:=]\s*[\"']?")
...
# form-encoded, HTML-embedded, and query-string values stay bounded. The scheme
# prefix is a chain (``Authorization: Basic Bearer sk``): consuming the run of
# scheme names in one match is what keeps the credential behind a doubled
# scheme from surviving as the next word after a non-overlapping match.
_ASSIGNMENT_VALUE_RE = re.compile(
    rf"(?:(?P<scheme>(?:{_AUTH_SCHEME})(?:\s+(?:{_AUTH_SCHEME}))*)\s+)?"
    r"(?P<value>[^\s&;,<>\"'?]+)",
    re.IGNORECASE,
)
...
# Matched case-insensitively: a proxy or logging layer may lowercase the header
# it reflects, so ``bearer``/``BASIC`` spellings must redact the same. An
# unambiguous scheme always cuts the value that follows -- a credential can be
# short and word-like, so neither its shape nor its vocabulary is trusted, and
# prose after a scheme name is conservatively lost rather than risked. The
# chain absorbs a whole run of scheme names (``Basic Bearer sk``) in one match,
# so a non-overlapping scan cannot stop at a doubled scheme and leave the
# credential behind it as unmatched text. Whitespace between the words includes
# newlines and tabs: display collapses whitespace only after redaction, so a
# credential on its own line must be caught before collapsing makes it adjacent
# to the scheme. Each chain repetition consumes at least one word and at most
# one word is given back when no value follows, so matching stays linear. The
# redaction marker is accepted as a value so a chain whose credential the
# assignment pass already cut (``Authorization: Basic Bearer sk``) is
# recognized as done rather than re-matched with a scheme word as the value.
_BARE_AUTH_SCHEME_RE = re.compile(
    rf"\b(?P<chain>(?:(?:{_AUTH_SCHEME})\s+)+)"
    rf"(?P<value>{re.escape(_REDACTED)}|[A-Za-z0-9\-._~+/=]+)",
    re.IGNORECASE,
)
_SCHEME_CHAIN_WORD_RE = re.compile(r"\S+")
# Scheme names that are also ordinary English words, compared case-insensitively:
# a bare occurrence needs a credential long enough that "Token 12345 expired" and
# "token count: 42" cannot trip it.
_AMBIGUOUS_BARE_SCHEMES = frozenset({"token", "oauth"})
_MIN_AMBIGUOUS_SCHEME_CREDENTIAL_CHARS = 16
# Challenge parameters (RFC 7235, plus the Digest and Bearer sets) follow a
# scheme name in a ``WWW-Authenticate`` challenge (``Basic realm="api"``) as
# directives, not as its credential. Matched as a prefix of the captured value
# so quoted (``realm="api"``, where the capture stops at the quote) and
# unquoted (``realm=api``) spellings are both kept. A credential cannot
# collide: base64 and JWT values never carry ``=`` before their final padding.
_AUTH_CHALLENGE_PARAM_RE = re.compile(
    r"(?:realm|charset|error|error_description|error_uri|scope|nonce|opaque"
    r"|qop|algorithm|stale|domain|userhash)=",
    re.IGNORECASE,
)
...
def _redact_assignments(text: str) -> str:
    ...
    useful and the credential is not; once an unambiguous scheme matches, in any
    letter case, the word that follows is cut regardless of shape or vocabulary,
    and a chained run of scheme names (``Basic Bearer sk``) keeps its scheme
    words while losing the first non-scheme token after the last of them -- an
    ambiguous word inside a chain that carries an explicit scheme
    (``Bearer token s3cret``) is scheme vocabulary, not the chain's end, so it
    cannot shield the token behind it. Only
    ``WWW-Authenticate`` challenge parameters (``realm=``, ``error=``,
    ``nonce=``) are exempt, by grammar rather than by value judgment, and the
    length heuristic remains only for the ambiguous scheme words ``Token`` and
    ``OAuth``.
    ...
    def _replace_bare_scheme(match: re.Match[str]) -> str:
        """Redact the token after a bare, possibly chained, auth scheme.

        A chain carrying at least one explicit scheme name always loses the
        first non-scheme token after it, whatever that token's shape: an
        ambiguous word such as ``token`` inside the chain is scheme vocabulary,
        and trusting it to end the chain would let ``Bearer token <secret>``
        keep the secret. A chain of only ambiguous words ("token count") needs
        a value long enough to be opaque, and a value shaped like a challenge
        parameter (``realm="api"``) is a directive rather than a credential.
        """

        chain, value = match.group("chain"), match.group("value")
        if value == _REDACTED or _AUTH_CHALLENGE_PARAM_RE.match(value):
            return match.group(0)
        has_explicit = any(
            word.lower() not in _AMBIGUOUS_BARE_SCHEMES
            for word in _SCHEME_CHAIN_WORD_RE.findall(chain)
        )
        if not has_explicit and len(value) < _MIN_AMBIGUOUS_SCHEME_CREDENTIAL_CHARS:
            return match.group(0)
        return f"{chain}{_REDACTED}"
