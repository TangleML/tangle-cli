...
from .api_transport import _content_to_text, sanitize_url
...
def format_http_error(exc: requests.HTTPError) -> str:
    ...
    needs to act on. The attempted URL goes through :func:`sanitize_url` and the
    body through :func:`_content_to_text`, the same transport-neutral redaction
    the diagnostic log path uses, so userinfo, credential query parameters, and
    credentials reflected in the body are removed *before* the body is collapsed
    to a single line and truncated -- truncating first would leave a secret in
    the surviving prefix. Only HTTP status failures are formatted here;
    ...
    if request is not None and request.url:
        target = f"{request.method} {sanitize_url(request.url)}"
    else:
        target = (sanitize_url(response.url) if response.url else "") or "Tangle API"
