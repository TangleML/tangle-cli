...
class ArtifactManager(TangleCliHandler):
    ...
    def _download_absolute_signed_url(signed_url: str) -> requests.Response:
        ...
        Transport trust is deliberately independent of the Tangle API client.
        The fetch is unauthenticated, so no Tangle token, auth header, or cookie
        reaches a third-party storage origin, and it verifies against Requests'
        standard system/environment trust. Tangle API TLS settings are not
        inherited: a Tangle-specific CA bundle must not become trusted for an
        unrelated storage origin, and disabling verification for a Tangle
        endpoint must not silently weaken signed-URL downloads. Self-hosted
        object stores remain configurable through the standard
        ``REQUESTS_CA_BUNDLE`` / ``CURL_CA_BUNDLE`` environment variables, which
        apply because ``verify`` is left unset here.
        """
