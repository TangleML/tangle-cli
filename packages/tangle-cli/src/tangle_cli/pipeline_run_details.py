...
def serialize_execution(execution: Any) -> dict[str, Any]:
    ...
    if component_spec:
        ...
        if implementation:
            out["implementation"] = _to_plain(implementation)
    arguments = _value(task_spec, "arguments")
    if arguments:
        out["arguments"] = _to_plain(arguments)
    ...
    if input_artifacts:
        out["input_artifacts"] = _to_plain(input_artifacts)
    output_artifacts = _value(execution, "output_artifacts")
    if output_artifacts:
        out["output_artifacts"] = _to_plain(output_artifacts)
    return out
...
def serialize_run_details(details: Any) -> dict[str, Any]:
    ...
    if annotations:
        out["run"]["annotations"] = _to_plain(annotations)
    if details.execution:
        out["execution"] = serialize_execution(details.execution)
    if details.annotations:
        out["annotations"] = _to_plain(details.annotations)
    if details.execution_state:
        out["execution_state"] = {
            "totals": _to_plain(_value(details.execution_state, "status_totals")),
            "per_execution": _to_plain(_value(details.execution_state, "child_execution_status_stats")),
        }
    return out
...
def get_run_details_output(
    ...
    *,
    ...
    details = client.get_run_details(run_id, **kwargs)
    return serialize_run_details(details)
...
def fetch_graph_state_one(client: Any, run_id: str) -> dict[str, Any]:
    ...
    state = client.executions_graph_execution_state(root_execution_id)
    return {
        "run_id": run_id,
        "root_execution_id": root_execution_id,
        "status_totals": _to_plain(_value(state, "status_totals")),
        "failed_execution_ids": _to_plain(_value(state, "failed_execution_ids")),
        "per_execution": _to_plain(_value(state, "per_execution")),
        "error": None,
    }
...
def get_graph_state_output(
    ...
    *,
    ...
    for run_id in run_ids:
        ...
        try:
            future = executor.submit(fetch_graph_state_one, client, run_id)
            try:
